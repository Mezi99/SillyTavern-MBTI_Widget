...
- **3.7.4** - Fix swipe deferral: v3.7.3's detection was too ST-version-specific to hold in practice. Its `textIsFinal()` treated only the literal `'...'` as in-progress, but ST does not reliably use that marker — a brand-new overswipe slot is pushed as `''` (or not yet present) on current releases, so `swipeVariantText()` returned `'' ≈ final` and `onMessageSwiped()` took the instant path, firing the analysis at swipe time with empty/stale text. The check is now the placeholder-agnostic `variantIsSettled()`: final only when the variant text is non-empty and not `'...'`/`'…'`, **or** when its `swipe_info[swipeIdx].gen_finished` is set (a genuinely empty-but-finished variant stays instant). Completion catching no longer depends on `GENERATION_ENDED` alone: the `MESSAGE_RECEIVED` handler now drains `settlePendingSwipe()` (primary signal — ST re-fires it with the real text after a swipe generation, and the early empty/placeholder emit is a no-op) *before* the auto-trigger runs, with `GENERATION_ENDED` retained as an idempotent backup. `onMessageSwiped()` records `pendingSwipe` for an unsettled swipe even while `isProcessing` (state only), so a swipe during a busy auto-trigger analysis is no longer swallowed, and `reAnalyzeLastTurn()`'s `finally` retries the drain once the busy flag releases — covering the rapid-swipe race with no recursion (the drain clears `pendingSwipe` before analyzing). A swipe that lands on a settled variant still discards stale `pendingSwipe`; `GENERATION_STOPPED` still clears it. Same prompt as always: `contextMessages` history + last user message + last AI (swiped/new) variant.
- **3.7.3** - Defer swipe analysis until the variant finishes generating. `MESSAGE_SWIPED` fires at *request* time, while a brand-new overswipe still shows ST's `'...'` placeholder — so analyzing immediately (v3.7.2) sent the placeholder instead of the finished AI reply. `onMessageSwiped()` now distinguishes the two cases via the new `textIsFinal()` helper (`swipeVariantText()` = `swipes[swipeIdx]` when stored, else `mes`; final = not `'...'`): a swipe between **existing** variants takes the instant path (cached restore or analyze) exactly as before, while a swipe to a **new** overswipe records `pendingSwipe = { userIdx, swipeId }` and returns. A new `GENERATION_ENDED` listener then runs `settlePendingSwipe()`: it analyzes via `reAnalyzeLastTurn({ force: true })` only when the pending turn is still the current tail (`userIdx` match), the active swipe is still the pending one (`swipeId` match — swiping away aborts the deferral), and the variant now holds real text (non-empty, not `'...'`), so the query always contains the finished variant text. A `GENERATION_STOPPED` listener clears the pending marker (a user-stopped swipe never completes), and any swipe that lands on a settled variant discards a stale `pendingSwipe`. The prompt sent is identical to the auto-trigger / manual re-analyze: `contextMessages` history lines + last user message (as stored in the chat file, edits included) + last AI (swiped/new) message. Both new listeners are wired with the same `event_types` existence guards as `MESSAGE_SWIPE_DELETED`. *(Superseded in v3.7.4: the `'...'`-only detection misclassified a new overswipe holding an empty `''` slot as final and fired the analysis at swipe time; `GENERATION_ENDED` alone missed completions. See 3.7.4.)*

- **3.7.2** - Swipe analysis. Listening for ST's `MESSAGE_SWIPED` event, the auto-trigger now fires whenever the user swipes to a new AI variant: `onMessageSwiped()` calls `reAnalyzeLastTurn({ force: true })` (force bypasses the "no new user message" guard — a swipe doesn't advance `userIdx`), which analyzes the just-shown variant and records it in the trail exactly like a fresh turn. Every analyzed variant is additionally cached per-chat in a new `mbti_swipes` metadata map `{[userIdx]: {[swipeCur]: {...}}}` (`cacheSwipeEntry()`), keyed to the swipe index captured at the *start* of the request so a mid-analysis swipe can't mis-key the result. Swiping back to an already-analyzed variant restores its analysis from the cache — `lookupSwipeCacheEntry()` + `upsertTrailEntry()` replay the tags/text with **no LLM call** (validated by a `fingerprint`: a djb2 hash of the exact variant text, so `MESSAGE_SWIPE_DELETED` index shifts automatically invalidate stale entries). Cache contents mirror the save toggles exactly: tag ratings always, `reasoning`/`commenter` text only when that prompt has Save on. `pruneSwipeCache()` (run on chat load plus `MESSAGE_SWIPE_DELETED`) drops entries whose user turn or variant text no longer exists; the cache key is written only while non-empty. `MESSAGE_SWIPE_ALL` is intentionally **not** handled — only the tail reply's swipe matters to the auto-trigger. New helpers `swipeIdOf()`, `swipeVariantFingerprint()`, `findAiReplyFor()` under the "Swipe analysis cache" banner.
...
```json
{
  "chat_metadata": {
    ...
    "mbti_swipes": {
      "10": { "0": { "tags": [...], "fingerprint": "x…", "analysisName": "Analysis", "reasoning": "…" }, "1": { "tags": [...], "fingerprint": "x…" } }
    }
  }
}
```
...
`mbti_swipes` (v3.7.2) is a **per-swipe variant cache**, distinct from the single active-swipe record in `mbti_trail`. Shaped `{[userIdx]: {[swipeCur]: {...}}}`, it stores the tag ratings (always) plus the saved prompt texts (only when that prompt's Save toggle is on) for every swipe variant that has actually been analyzed — so swiping back to an already-analyzed variant restores its analysis instantly instead of burning another LLM call. Each variant record carries a `fingerprint` (djb2 hash of the exact variant text) that lets readers reject stale entries when swipes are deleted and their indices shift. The cache is written by `cacheSwipeEntry()` from `reAnalyzeLastTurn()` (so the auto-trigger's first variant is always primed), restored by `lookupSwipeCacheEntry()` + `upsertTrailEntry()` in the `MESSAGE_SWIPED` handler (`onMessageSwiped()`), and pruned by `pruneSwipeCache()` on chat load and `MESSAGE_SWIPE_DELETED`. Since v3.7.4, an analysis for a brand-new overswipe is deferred (`pendingSwipe`) until the variant holds final content — drained by `MESSAGE_RECEIVED` (primary; the placeholder emit is a no-op) or `GENERATION_ENDED` (backup) — so the cache is only ever primed with real variant content, never an empty slot. Omitting the cache when it is empty keeps branched chats free of the key.
