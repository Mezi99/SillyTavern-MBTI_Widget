...
**Re-scan output scaling:** The re-scan prompt asks the model to return one analysis entry per user message (tags + reasoning), so output grows with user message count. `getRescanOutputBudget(userMessageCount, inputEstimate, contextBudget)` floors at `max(configured maxTokens, userMessageCount × 160)` and caps at the remaining context (`context − inputEstimate`, ≥1024) — the largest output that still fits — so reasoning-model "thinking" is never truncated. If the custom API still returns empty content despite the scaling, the error includes `finish_reason` and response structure details for diagnosis.
