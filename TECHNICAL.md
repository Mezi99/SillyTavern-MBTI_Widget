...
- `fitChunks(messages, budget)` — contiguous oldest-first packing returning `[{ messages, inputTokens }]`; used by both the scanner and the popup preview so the plan always matches the actual scan.
